...
public class ProductService {
    ...
    @Transactional
    public ProductResponse create(ProductRequest request, Authentication admin) {
        ...
        auditLogService.log(admin.getName(), "PRODUCT_CREATED", "Product '" + saved.getName() + "' (id=" + saved.getId() + ")");
        return toResponse(saved);
    }

    @Transactional
    public ProductResponse update(Long id, ProductRequest request, Authentication admin) {
        ...
        Product saved = productRepository.save(product);
        auditLogService.log(admin.getName(), "PRODUCT_UPDATED", "Product id=" + id + " updated");
        return toResponse(saved);
    }

    @Transactional
    public void delete(Long id, Authentication admin) {
        Product product = findProductOrThrow(id);
        product.setActive(false); // soft delete preserves order history integrity
        productRepository.save(product);
        auditLogService.log(admin.getName(), "PRODUCT_DEACTIVATED", "Product id=" + id + " deactivated");
    }

    public long countLowStock() {
        return productRepository.countByStockQuantityLessThan(LOW_STOCK_THRESHOLD);
    }
