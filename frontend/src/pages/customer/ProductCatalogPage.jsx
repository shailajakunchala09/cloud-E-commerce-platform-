      ...
      <div className="flex-between mb-16" style={{ flexWrap: "wrap", gap: 12 }}>
        ...
        <form onSubmit={handleSearch} className="flex" style={{ gap: 8 }}>
          <input
            ...
            style={{ padding: "9px 12px", borderRadius: 8, border: "1px solid var(--border)", width: 240 }}
          />
          <button className="btn btn-primary" type="submit">Search</button>
        </form>
      </div>

      <div className="flex mb-16" style={{ gap: 8, flexWrap: "wrap" }}>
        <button
          className={`btn ${!activeCategory ? "btn-primary" : "btn-outline"}`}
          onClick={() => { setActiveCategory(null); setKeyword(""); setPage(0); }}
        >
          All
        </button>
        {categories.map((c) => (
          <button
            ...
            onClick={() => { setActiveCategory(c.id); setKeyword(""); setPage(0); }}
          >
            {c.name}
          </button>
        ))}
      ...
      {loading ? (
        ...
        <>
          ...
          {pageData?.content?.length === 0 && (
            <p className="muted text-center" style={{ padding: 48 }}>No products found.</p>
          )}
          ...
          {pageData && pageData.totalPages > 1 && (
            <div className="flex-between mt-24">
              ...
              <span className="muted">Page {page + 1} of {pageData.totalPages}</span>
              <button
